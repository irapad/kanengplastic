...
const ProductShowcase = () => {
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesSearch = product.name.toLowerCase().includes(term) || product.description.toLowerCase().includes(term) || product.features.toLowerCase().includes(term);
    const matchesCategory = selectedCategory === "ทั้งหมด" || product.category === selectedCategory;
    return matchesSearch && matchesCategory;
  });
  return <section id="products" className="section-container bg-background">
      <div className="container mx-auto">
        ...
        <div className="mb-8 space-y-4 fade-in">
          <div className="relative max-w-md mx-auto">
            ...
            <input type="text" placeholder="ค้นหาสินค้า..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-12 pr-6 py-4 rounded-2xl border-2 border-gray-600 bg-gray-700 backdrop-blur-lg shadow-xl shadow-black/20 focus:outline-none focus:ring-4 focus:ring-gray-500/50 focus:border-gray-400 focus:shadow-2xl focus:shadow-black/30 transition-all duration-300 text-lg text-white placeholder:text-gray-300 hover:bg-gray-600" />
          </div>

          <div className="flex flex-wrap justify-center gap-2">
            {categories.map(category => <button key={category} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 rounded-full text-sm transition-all ${selectedCategory === category ? "bg-primary text-primary-foreground shadow-lg scale-105" : "bg-card/50 text-foreground hover:bg-primary/10 hover:scale-105"}`}>
                {category}
              </button>)}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((product, index) => <div key={product.id} className={`product-card group fade-in stagger-${index % 6 + 1}`}>
              {/* Badge */}
              {product.popular && <div className="absolute top-4 right-4 bg-primary text-primary-foreground text-xs px-2 py-1 rounded-full z-10">
                  ยอดนิยม
                </div>}
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <div className="relative mb-4 overflow-hidden rounded-xl cursor-pointer group/image">
                    <img src={product.image} alt={product.name} loading="lazy" className="w-full h-48 object-cover transition-transform duration-500 group-hover:scale-110" />
                ...
                <DialogContent className="max-w-4xl w-full p-0 bg-background border-0">
                  <div className="relative">
                    <img src={product.image} alt={product.name} className="w-full max-h-[80vh] object-contain rounded-lg" />
        ...
        {/* No Results */}
        {filteredProducts.length === 0 && <div className="text-center py-12 fade-in">
